...
let data,board,period='yesterday',selectedBin=null;
...
function renderPeriod(){const p=data[period], prior=data[period==='yesterday'?'previous_day':'previous_week'],day=period==='yesterday';
 ...
 const max=Math.max(1,...p.bins.map(b=>Math.abs(b.growth||0)));$('#chart').innerHTML=p.bins.map((b,i)=>'<div class="column"><button data-bin="'+i+'" class="'+(b.growth==null?'missing ':'')+(selectedBin===i?'selected':'')+'" style="height:'+Math.max(2,Math.abs(b.growth||0)/max*135)+'px" title="'+esc(b.label+'：'+N(b.growth))+'"></button><small>'+esc(day?b.label.slice(0,2):b.label)+'</small></div>').join('');
 $('#chart').querySelectorAll('[data-bin]').forEach(b=>b.onclick=()=>{selectedBin=+b.dataset.bin;renderPeriod()});
 ...
 const chosen=selectedBin!=null?p.bins[selectedBin]:top[0];$('#contribution').innerHTML=chosen?'<h3>'+esc(chosen.label)+' · 主要贡献视频</h3>'+table(['手机 / 账号','PID','时段增长'],chosen.contributors.map(v=>['#'+v.account_no+' '+esc(v.username),esc(v.video_id),N(v.growth)])):'<div class="empty">点选柱形查看贡献视频</div>';
...
function renderOther(){
 ...
 $('#rewards').innerHTML=table(['手机 / 账号','累计估算收益','核对时间','计划状态'],a.map(x=>['<span class="phone">#'+x.account_no+'</span> '+esc(x.username)+(x.test?'<br><small>测试赛道</small>':''),money(x.amount),D(x.checked_at),'<span class="badge '+(x.program==='joined'?'good':'')+'">'+STATES[x.program]+'</span>']));
...
document.querySelectorAll('[data-period]').forEach(b=>b.onclick=()=>{period=b.dataset.period;selectedBin=null;document.querySelectorAll('[data-period]').forEach(x=>x.classList.toggle('active',x===b));if(data)renderPeriod()});
